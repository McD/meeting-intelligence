...
- **[Claude Code](https://claude.com/claude-code) account** — Pro, Max, Team, or Enterprise all work. The scheduler invokes Claude Code headless (`claude -p`), which any paid Claude plan supports. Max is recommended for heavy use since Pro's usage limits are tighter; a raw API key alone is not enough.
...
- **Paid Claude Code account required.** The scheduler runs `claude -p` headless on each active cycle. Pro, Max, Team, and Enterprise all support this; the free Claude.ai plan does not, and raw API keys are not used. For heavy meeting days, Max avoids running into Pro's tighter usage limits.
...
- `expand: <request>` — re-fetches the meeting transcript (Gemini Doc, Teams Recap, or local MacWhisper file) and runs Claude with your specific ask. Example: `expand: write up the industry overview as a one-pager`. You can also paste a transcript inline below the `expand:` line (≥500 chars; line breaks not required, single-paragraph pastes work) — handy on briefing threads, where the original meeting hasn't happened yet but you want to use a transcript from a different meeting. The result lands as a reply in the same email thread (and Slack channel if `~/.slack_webhook` is configured).
- `quote: <topic>` — returns 3 to 6 direct quotes from the transcript where speakers discuss the topic. Same transcript resolution as `expand:` (inline-paste, then `transcript_source`). Useful for pulling out the actual lines someone said about a thing.
