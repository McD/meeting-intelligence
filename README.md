...
- `expand: <request>` — re-fetches the meeting transcript (Gemini Doc, Teams Recap, or local MacWhisper file) and runs Claude with your specific ask. Example: `expand: write up the industry overview as a one-pager`. You can also paste a transcript inline below the `expand:` line (≥500 chars, ≥10 lines) — handy on briefing threads, where the original meeting hasn't happened yet but you want to use a transcript from a different meeting. The result lands as a reply in the same email thread (and Slack channel if `~/.slack_webhook` is configured).
