...
**Email** — to `$MY_EMAIL` using `--html`, subject: `Actions tracker — <Day> <DD Mon> <YYYY>`. Convert the markdown to HTML using the same Python snippet as `commands/follow-up.md` Step 6 (re-derive `$HTML` from `$DIGEST_FILE`). Capture the `threadId` from the send response.
