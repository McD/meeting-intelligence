...
**Non-negotiable invariant.** Every follow-up cycle MUST reach a defensible statement about every awaiting file: either (a) the state file has been processed for any new user reply on the thread since `last_processed_msg`, or (b) it has been verified that the thread has no message newer than `last_processed_msg` (or has only the initial bot send). There is no third option. In particular:

1. **"No TTL expiries → skip Step 0" is banned.** TTL expiry is one of the checks inside Step 0; the absence of expired files says nothing about pending replies. If the summary sentence for Step 0 mentions only TTL, dispatch cadence, or scheduler intervals, Step 0 was not run and the cycle is incorrect.
2. **"Dispatch is on the scheduler's 15-min cadence, so this cycle can defer" is banned.** This cycle IS the scheduler cadence. There is no other process. Deferring is a null action.
3. **The `from:mark@screencloud.io newer_than:1h` optimization** (referred to in prior logs as the "delta gate") is not defined in this skill. If used, it must satisfy ALL of: (i) the query window is ≥ time since the previous Step 0 for this scheduler cadence plus a 15-min safety margin (never `newer_than:1h` when this cycle follows a sleep-suppressed cycle — extend to cover the gap); (ii) if the query returns any Gmail message whose thread ID matches any awaiting-file `thread_id`, per-file thread checks are MANDATORY for those files; (iii) the query result count and the list of matched thread IDs are both logged. Any cycle that has been preceded by a sleep-suppression skip (visible as an `INFO: … skipped (host-sleep majority …)` entry in `scheduler.log` since the last completed Step 0) MUST fall back to per-file thread checks — the optimization's window guarantee doesn't hold across sleep windows.
4. **Per-file thread checks are the default.** Every branch below (retirement, awaiting-reply, awaiting-digest, transcript-request) starts with a `gws gmail users threads get` for the file's `thread_id`. That is the load-bearing operation. Optimizations only skip files known to have no new activity — they do not skip the branch's semantics.
...
5. **Parse the first command line.** Take the first non-empty, non-quoted line of the reply body (strip `>`-prefixed quoted-original lines first — same convention as the transcript-request branch). Lowercase the keyword prefix only (preserve case in any argument that follows).
   ...
   **Convention every send-branch below follows.** When a branch sends an email via `gws gmail +send`, capture the response's `id` field into `BOT_REPLY_MESSAGE_ID`. The Step 5b atomic rewrite consumes this variable and appends it to `bot_sent_ids`, so the next dispatcher cycle silently skips the bot's own message instead of trying to parse it as a user reply. Branches that send no email (e.g., `extend`, `cancel`) set `BOT_REPLY_MESSAGE_ID=""` and the array is preserved unchanged.
   ...
   - `expand: <request>` — focused re-run against the transcript:
     1. **Check for an inline transcript first.** Take the reply body, strip `>`-prefixed quoted-original lines, drop the keyword line itself, and drop the Gmail `On [date], ... wrote:` quote-marker line plus everything after it. If the remaining body has **≥500 characters**, treat that text as the transcript and skip step 2 — regardless of line count, since some mail clients (Apple Mail, certain web clients) collapse a pasted transcript into a single unbroken paragraph and a line-count gate would wrongly reject it. This is the common case for briefing-thread `expand:` replies, where the user pastes a transcript from a separate meeting alongside the request (e.g. `Expand: my narrative based on this Elizabeth conversation. Transcript below\n\n<pasted transcript>`). Set `$TRANSCRIPT_SOURCE_FOR_REPLY` to the Gmail thread URL of the user's reply so the Source section in step 4's response can link back to the email containing the pasted transcript.
     2. **Otherwise fetch from `transcript_source`** using the same five-branch logic as Step 2 (Google Doc via `gws drive`, Gmail thread via `gws gmail`, local `file://` via direct read).
     3. If neither path yields a transcript (no substantial inline body AND fetch fails: 404, file missing, or `transcript_source` is empty — the normal case for briefing-thread replies that don't paste a transcript), send an email reply to `$thread_id` saying "`expand:` needs a meeting transcript, and this thread doesn't have one yet (either the meeting hasn't happened, or the original transcript is no longer reachable at `<transcript_source>`). Paste a transcript inline below an `expand:` line if you want to use one from a different meeting, try `research: <query>` for web research instead, or `cancel` to drop this thread." Log a one-line WARN.
     4. Otherwise, run a focused Claude pass with the transcript as context and the user's `<request>` as the instruction. Aim for 200–800 words unless the request explicitly asks for more. Format as plain prose or short bulleted lists — match the spirit of the ask. Don't add scaffolding (no executive summaries, table of contents, or meta-commentary).
     5. Send the result as an email reply to the same thread: `gws gmail +send --thread-id "$thread_id" --subject "Re: Follow-up: [meeting]" --body "$RESULT_HTML" --html`. Also Slack-mirror if `~/.slack_webhook` exists.
     6. Log `"Expand request handled for [meeting]: [first 60 chars of request]"` (and append `" (inline transcript, N chars)"` when step 1 fired instead of step 2, so the log distinguishes the two paths).
   ...
   - `quote: <topic>` — extract direct quotes:
     1. Resolve the transcript the same way as `expand:` — inline-paste first (≥500 chars after stripping quoted-original and the keyword line; no line-count gate, since single-paragraph pastes are common), then `transcript_source` as fallback.
     2. If neither yields a transcript, send the same graceful "transcript not available" reply as `expand:` step 3.
